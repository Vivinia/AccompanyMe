...
import com.example.weixu.accompanyme.MusicActivity;
import com.example.weixu.music.handler.SimpleCallbackHandler;
import com.example.weixu.music.request.NeteaseCloud;
import com.example.weixu.music.response.NeteaseCloudPlayerUrlResponse;
...
public class NeteaseCloudSongAdapter extends RecyclerView.Adapter {
    ...
    private void playMusic(final NeteaseCloudSearchSongResponse.Result.SongItem song) {
        ...
        ((MusicActivity)activity).httpClient.newCall(req).enqueue(new SimpleCallbackHandler<NeteaseCloudPlayerUrlResponse>(activity) {
            @Override
            public void onResult(Call call, NeteaseCloudPlayerUrlResponse response) {
                ...
                activity.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        ((MusicActivity)activity).playMusic(songItem);
                    }
                });
            }
        });
